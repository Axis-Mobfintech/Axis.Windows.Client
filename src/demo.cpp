...
static void registro_do_validador_ ( void )
{
   ...
   registrerDevice.set_allocated_transaction_date ( date_time );
   ...
   std::cout
      ...
      << "  date         : " << TimeUtil::ToString ( response.transaction_date ( ) ) << std::endl;
}
...
static void requisicao_de_parametros_ ( void )
{
   ...
   date_time->set_seconds ( time ( NULL ) );
   date_time->set_nanos ( 0 );
   parametres.set_allocated_transaction_date ( date_time );
...
static void log_requisicao_lista_excessao_ ( RestrictionListResponse response )
{
   ...
   // Restrições
   for ( i = 0; i < response.restriction_list_size ( ); i++ )
   {
      RestrictionListCardInformation card_info = response.restriction_list ( i );
...
static void log_requisicao_lista_aceitacao_ ( AcceptanceListResponse response )
{
   ...
   // Lista de aceitos
   for ( i = 0; i < response.acceptance_list_size ( ); i++ )
   {
      AcceptanceListCardInformation card_info = response.acceptance_list ( i );
